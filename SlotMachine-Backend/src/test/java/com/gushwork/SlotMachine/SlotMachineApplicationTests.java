...
class SlotMachineApplicationTests {
	...
	@BeforeEach
	public void setUp() {
		user = new User("test", "test@abc.com");
		testSession = new Session(user);
	}

	@Test
	public void testCreateSession() {
		// When repository.createSession() is called, return a new test session.
		when(sessionRepository.createSession(user)).thenReturn(testSession);
		Session session = slotMachineService.createSession(user);
		assertNotNull(session.getSessionId());
		assertEquals(10, session.getCredit());
		verify(sessionRepository, times(1)).createSession(user);
	}
	...
	public void testRollWinningOutcome() {
		...
		when(rollStrategy.shouldRoll(testSession.getCredit())).thenReturn(false);
		...
		assertEquals(19, testSession.getCredit());
	}
	...
	public void testRollLosingOutcome() {
		...
		// Credits: 10 - 1 = 9.
		assertEquals(9, testSession.getCredit());
	}
	...
	public void testRollWithForcedRoll() {
		...
		when(rollStrategy.shouldRoll(testSession.getCredit())).thenReturn(true);
		...
		assertEquals(49, testSession.getCredit());
	}
